...
(function(html) {
    ...
    * ------------------------------------------------------ */
    const ssMoveTo = function() {
        ...
        const triggers = document.querySelectorAll('.back-to-top-scroll');
    ...
    // Craigslist back to top button
    
document.addEventListener('DOMContentLoaded', function() {
  const clGoTop = document.querySelector('.cl-go-top');

  if (clGoTop) {
    window.addEventListener('scroll', () => {
      if (window.scrollY > 4000) {  
        clGoTop.style.display = 'flex';
      } else {
        clGoTop.style.display = 'none';
      }
    });
  }
});

// Back to top arrow changes to black on light page, and white on dark page
const clGoTop = document.querySelector('.cl-go-top');
const clBackToTop = document.querySelector('.cl-back-to-top');
const clBackToTopText = document.querySelector('.cl-go-top span');
...
window.addEventListener('scroll', () => {
  ...
  // Change color when reaching footer
  if (window.scrollY + window.innerHeight >= footer.offsetTop) {
    clBackToTop.querySelector('svg path').style.fill = 'white';
    clBackToTop.style.borderColor = 'white';
    clBackToTopText.style.color = 'white';
  } else {
    clBackToTop.querySelector('svg path').style.fill = 'black';
    clBackToTop.style.borderColor = 'black';
    clBackToTopText.style.color = 'black';
  }
});
